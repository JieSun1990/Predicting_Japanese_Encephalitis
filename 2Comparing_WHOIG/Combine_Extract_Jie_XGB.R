...
dir.create(file.path('Generate/XGB'), showWarnings = TRUE) #@@
...
Cases.map <- readRDS(paste0(LinkData, "Generate/Cases/XGB/", ListFiles[length(ListFiles)])) # Take total cases (index 101) #@@
...
p <- p + ggtitle('Case Compare Mapping vs WHO Incidence Group \nXGB') +  xlab('Countries') + ylab('Cases') #@@
p

cat('===== FINISH [Extract_Cases_Country.R] =====\n')
# SAVE FILE
ggsave(filename = 'Generate/XGB/Cases_Comparison.png', width = 108*2.5, height = 72*2.5, units = 'mm', plot = p)
...
# Create folder to store the result (will show warnings if the folder already exists --> but just warning, no problem)
dir.create(file.path('Generate/GB/FOI_XGB/'), showWarnings = TRUE) #@@
Savepath <- 'Generate/XGB/' #@@
...
FOI_Estimated <- read.csv(paste0(LinkData, "Data/Endemic_FOI_XGB_Full_Cov_400.csv"),sep = '\t')#@@
...
saveRDS(result.country, paste0(Savepath, 'Mapping_FOI_Distribution_Countries_XGB.Rds'))
...
for (country in All_Countries){
  ...
  ggsave(filename = paste0('Generate/XGB/FOI_XGB/FOI_Dens_', country, '.png'), width = 108*2.5, height = 72*2.5, units = 'mm', plot = p3) #@@
...
cat('===== START [Extract_Cases_Country_XGB.R] =====\n') #@@
...
cat('===== FINISH [Extract_Cases_Country_XGB.R] =====\n') #@@

# SAVE FILE
ggsave(filename = 'Generate/XGB/Cases_XGB.png', width = 108*2.5, height = 72*2.5, units = 'mm', plot = p) #@@
