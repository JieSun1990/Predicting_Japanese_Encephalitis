...
dir.create(file.path('Generate/RF'), showWarnings = TRUE) #@@
...
Link_Generate_Cases_DF <- paste0(LinkData, 'Generate/Cases/RF/') #@@
...
Cases.map <- readRDS(paste0(LinkData, "Generate/Cases/RF/", ListFiles[length(ListFiles)])) # Take total cases (index 101) #@@
...
p <- p + ggtitle('Case Compare Mapping vs WHO Incidence Group \nRF') +  xlab('Countries') + ylab('Cases') #@@
p

cat('===== FINISH [Extract_Cases_Country.R] =====\n')
# SAVE FILE
ggsave(filename = 'Generate/RF/Cases_Comparison.png', width = 108*2.5, height = 72*2.5, units = 'mm', plot = p)
...
# Create folder to store the result (will show warnings if the folder already exists --> but just warning, no problem)
dir.create(file.path('Generate/RF/FOI_RF/'), showWarnings = TRUE) #@@
Savepath <- 'Generate/RF/' #@@
...
FOI_Estimated <- read.csv(paste0(LinkData, "Data/Endemic_FOI_RF_Quantile_Regression_Full_Cov_400.csv"),sep = '\t')#@@
...
saveRDS(result.country, paste0(Savepath, 'Mapping_FOI_Distribution_Countries_RF.Rds'))
...
for (country in All_Countries){
  ...
  ggsave(filename = paste0('Generate/RF/FOI_RF/FOI_Dens_', country, '.png'), width = 108*2.5, height = 72*2.5, units = 'mm', plot = p3) #@@
...
# SAVE FILE
ggsave(filename = 'Generate/RF/Cases_RF.png', width = 108*2.5, height = 72*2.5, units = 'mm', plot = p) #@@
