...
dir.create(file.path('Generate/Cases/GB/'), showWarnings = TRUE)

####### Generate Cases Pixel DF #######
# Read FOI Map file
df.csv <- read.csv('Data/Endemic_FOI_GB_Quantile Regression_Full_Cov_400.csv', sep = '\t') #@@place to change file
df.csv <- df.csv[, -1]
...
for (i in 3 : ncol(df.casesage)){
  ...
  saveRDS(df, paste0('Generate/Cases/GB/Cases_', colnames(df.casesage)[i],m_name, '.Rds')) #@@
}
...
dir.create(file.path('Generate/Cases_TIF/GB/'), showWarnings = TRUE) #@@
...
Namefile = paste0('Generate/Cases_TIF/GB/Cases_', colnames(df)[3], m_name) #@@
...
dir.create(file.path('Generate/Cases_SHP/GB'), showWarnings = TRUE) #@@
...
writeOGR(region.shp, ".", "Generate/Cases_SHP/GB/Total_Cases_SHP", driver="ESRI Shapefile") #@@ SJ: it's not saving to the Cases_SHP folder
